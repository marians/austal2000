// ====================================================== verif51a.c
//
// Evaluation of verification 51a
// ==============================
//
...
int main( int argc, char *argv[] ) {
  ...
  int i, j, k, n, l;
  double c, x, z, csum, zsum, sz, za, szth, zath, hmax, t;
  FILE *f;
  ...
  for (i=Iq+Di; i<=Iq+Nn; i+=Di) {
    ...
    if (csum > 0)  zsum /= csum;
    sz = sqrt(zsum);
    zath = 2.84*pow(Qq, 0.333)*pow(x, 0.667)/U;
    hmax = 102*pow(Qq, 0.6)/U;
    if (zath > hmax)  zath = hmax;
    zath += H;
    t = x/U;
  ...
  fprintf(Out, "\n");                                             //-2008-09-30
  vMsg("@verif51a finished");                                     //-2008-09-30
  return 0;
}
