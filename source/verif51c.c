// ====================================================== verif51c.c
//
// Evaluation of verification 51c
// ==============================
//
...
static char *Path = "verif/51c";
static char FileCnc[64];
static char *NmOutput = "verif/51c/VDIOUT.DAT";
static int Nx=100, Iq=2, Ny=3, Jq=2, Nz=40, Nn=70;
static double *Cc;
...
int main( int argc, char *argv[] ) {
  ...
  int i, j, k, n, l, m, drop;
  double a, c, x, h, z, csum, zsum, za, zath;
  FILE *f;
  ...
  f = fopen(NmOutput, "r");
  if (!f) {
    vMsg(_cant_read_$_, NmOutput);                                //-2008-09-30
    exit(10);
  }
  i = 0;
  drop = 1;
  while(fgets(buf, 400, f)) {
    ...
    if (strstr(buf, "DISTANCE"))  continue;
    if (2 != sscanf(buf, "%lf %lf", &x, &h)) {
      vMsg(_read_error_$_, i+1);                                  //-2008-09-30
      fclose(f);
      exit(11);
    }
    if ((Xv) && (Hv) && (n<MAXVAL)) {
  ...
  for (i=Iq+Di; i<=Iq+Nn; i+=Di) {
    ...
    if (m < Nv) {
      a = (x-Xv[m-1])/(Xv[m]-Xv[m-1]);
      zath = H + Hv[m-1] + (1-a)*(Hv[m]-Hv[m-1]);
    }
    else  zath = H + Hv[Nv-1];
    fprintf(Out, " %7.1f %7.1f\n", za, zath);                     //-2008-09-30
  }
  fprintf(Out, "\n");                                             //-2008-09-30
  vMsg("@verif51c finished");                                     //-2008-09-30
  return 0;
}
